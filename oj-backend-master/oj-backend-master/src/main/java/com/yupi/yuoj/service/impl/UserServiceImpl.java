...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public LoginUserVO userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        // 3. 记录用户的登录态（只存 userId：敏感字段不下发，Redis 不落库，每次请求从数据库取最新数据）
        request.getSession().setAttribute(USER_LOGIN_STATE, user.getId());
        return this.getLoginUserVO(user);
    }

    /**
     * 从 session 中读取登录用户 id
     * 兼容旧版本直接存入完整 User 对象的情况，统一收敛为 id
     *
     * @param request
     * @return 未登录返回 null
     */
    private Long getLoginUserId(HttpServletRequest request) {
        ...
        if (userObj instanceof Number) {
            return ((Number) userObj).longValue();
        }
    ...
    public User getLoginUser(HttpServletRequest request) {
        ...
        // session 只存了 userId，每次从数据库查询最新用户信息（含最新角色/状态）
